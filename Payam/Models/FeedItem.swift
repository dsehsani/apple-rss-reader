...
extension FeedItem {
    ...
    ///   - clusterSize: Total members in this item's cluster (caller computes via
    ///     grouping on `clusterID`). Defaults to 1 for solo items.
    /// - Returns: An Article suitable for ArticleCardView and related UI.
    func toArticle(categoryID: UUID, clusterSize: Int = 1) -> Article {
        ...
        return Article(
            ...
            isCanonical: clusterID == nil ? true : isCanonical
        )
    }

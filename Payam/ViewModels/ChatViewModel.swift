...
@Observable final class ChatViewModel {
    ...
    var welcomeMessage: String {
        ...
        return "Hi! I'm your Payam assistant. I can summarize articles, suggest RSS feeds, or help you use the app."
    }
    ...
    func send() async {
        ...
        do {
            let agentContext = articleContext.map {
                AgentClient.ArticleContext(title: $0.title, feedName: $0.feedName, content: $0.content)
            }
            let envelope = try await AgentClient.send(
                history: messages,
                articleContext: agentContext,
                subscriptions: []
            )
            let reply: String
            ...
            default:
                reply = "I found something — try asking from the main assistant for full results."
            }

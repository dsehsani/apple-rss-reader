...
final class RiverViewModel {
    ...
    func refresh() async {
        ...
        // Load filter rules + scope map on MainActor. We deliberately reach into
        // SwiftDataService.shared (bypassing the FeedDataService protocol) because
        // filter rules are a SwiftData-only feature; mock data services for previews
        // don't need to surface them.
        let filterRules = SwiftDataService.shared.loadFilterRuleSnapshots()
        let folderNameByID: [UUID: String] = Dictionary(
            uniqueKeysWithValues: dataService.categories.map { ($0.id, $0.name) }
        )
        let sourceFilterMeta: [UUID: (feedURL: String, folderName: String?)] = Dictionary(
            uniqueKeysWithValues: sources.map { src in
                (src.id, (src.feedURL, folderNameByID[src.categoryID]))
            }
        )
        ...
        if let sds = dataService as? SwiftDataService {
            // fetchAllRecentItems intentionally returns hidden items so the
            // 30-day retention window stays intact for source/folder views.
            // Pipeline Phase A dedup marks duplicates with river_visible=0,
            // so we filter those out before exposing to legacy views.
            let feedItems = SQLiteStore.shared.fetchAllRecentItems()
                .filter { $0.riverVisible }

            // Precompute cluster sizes so legacy views (FolderViewModel,
            // ArticleCardView) can render the "+N" cluster indicator. The
            // pipeline only persists clusterID/isCanonical per item — size
            // is derived from grouping.
            let clusterSizes: [UUID: Int] = feedItems
                .compactMap(\.clusterID)
                .reduce(into: [:]) { acc, cid in acc[cid, default: 0] += 1 }

    ...
    var body: some View {
        ...
        .sheet(isPresented: $showChatSheet) {
            AgentSheetView(viewModel: agentViewModel)
                ...
                .onAppear {
                    // Wire SwiftData's current subscriptions into the agent on each
                    // sheet open. The closure captures `SwiftDataService.shared` so
                    // every call sees fresh state without an explicit reload.
                    agentViewModel.subscriptionsProvider = {
                        SwiftDataService.shared.sources.map { src in
                            AgentClient.SubscriptionRef(title: src.name, feedURL: src.feedURL)
                        }
                    }
                }
        }

...
| File | Purpose |
|---|---|
| `Payam/Models/UserProfile.swift` | SwiftData model — stores `appleUserID`, `displayName`, `emailRelay`, `syncEnabled`, `lastSyncedAt` |
| `Payam/Services/AuthenticationManager.swift` | `@Observable` singleton — manages Sign in with Apple lifecycle, credential checks, guest mode |
| `Payam/Services/KeychainService.swift` | Secure Keychain wrapper — persists Apple user ID across reinstalls |
| `Payam/Views/Onboarding/OnboardingView.swift` | Full-screen welcome screen with Sign in with Apple + "Continue without account" |
| `Payam/Views/Settings/AccountView.swift` | Account management sheet — profile card, sync status, sign-out |

---

## Critical Warning: Do NOT Do a Straight Merge

The auth branch contains an **older version of `PayamApp.swift`** and **an older version of `SettingsView.swift`** that are missing work already done on `main`. A straight `git merge` will produce conflicts, and accepting the branch version wholesale will **delete**:
...
```bash
git checkout origin/feature/auth-phase1 -- Payam/Models/UserProfile.swift
git checkout origin/feature/auth-phase1 -- Payam/Services/AuthenticationManager.swift
git checkout origin/feature/auth-phase1 -- Payam/Services/KeychainService.swift
git checkout origin/feature/auth-phase1 -- Payam/Views/Onboarding/OnboardingView.swift
git checkout origin/feature/auth-phase1 -- Payam/Views/Settings/AccountView.swift
```

These files are clean additions. Add them to the Xcode project target (Payam target, not tests).

---

## Step 3: Update PayamApp.swift

This is the most important conflict. The auth branch's `PayamApp.swift` stripped all BGTask code. The final file must have **both** the auth additions AND the background task code from `main`.

Apply these changes to the **current `main` version** of `PayamApp.swift`:
...
The following methods must remain untouched in `PayamApp.swift`. The auth branch deleted them — restore from `main` if they were lost:
...
1. Open `Payam.xcodeproj`
2. Select the `Payam` target → **Signing & Capabilities**
3. Click **+ Capability**
...
1. **Schema includes `UserProfile`** — check `PayamApp.init()` has all four models in the `Schema([...])` call
...
- Never overwrite `PayamApp.swift` with the auth branch version wholesale. The auth branch version is missing ~80 lines of BGTask code.

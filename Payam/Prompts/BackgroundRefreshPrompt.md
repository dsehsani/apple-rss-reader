...
The background refresh is already scaffolded in `PayamApp.swift`. Here is exactly what exists and what is missing:
...
```
Payam/PayamApp.swift                     ← scheduling logic
Payam/Views/Settings/SettingsView.swift    ← wire picker + show last-updated
```

One new file:

```
Payam/Services/RefreshStateStore.swift     ← UserDefaults wrapper for refresh state
```
...
Create this file at `Payam/Services/RefreshStateStore.swift`.

This is a lightweight `@Observable` UserDefaults wrapper. It is the single source of truth for refresh interval and last-refresh timestamp. Both `PayamApp` and `SettingsView` read from it.
...
Add this static method inside `PayamApp`, below `handleRiverRefresh`:
...
> **Note:** Add `import Network` to the top of `PayamApp.swift` alongside the existing imports.
...
- `scheduleNextRiverRefresh()` is called both from `handleRiverRefresh` (at the start of the background task) and from the `didEnterBackgroundNotification` handler. Both call paths already exist — no structural changes to `PayamApp.body` are needed.

...
Create this file at `Payam/Services/OPMLService.swift`.
...
final class OPMLService: NSObject {
    ...
    func export(folders: [FolderModel], unfiledFeeds: [FeedModel]) throws -> URL {
        ...
        <opml version="2.0">
          <head>
            <title>Payam Subscriptions</title>
...
If `SwiftDataService.shared` is already injected at the root (in `PayamApp.body`), it propagates automatically and no change is needed.

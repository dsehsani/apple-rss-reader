...
**Payam — Target: Reduce ~6s pipeline to <2s perceived load time**

---

## Context for Claude

This is the **Payam** iOS SwiftUI app. When a user taps an article, the text rendering pipeline currently takes ~6 seconds before content is visible. The pipeline runs entirely on-device.
...
actor WebViewPool {
    ...
    /// Call this at app launch (e.g., in PayamApp.init or AppState.init).
...
**Wire it up in `PayamApp.swift` or `AppState.swift`:**
```swift
init() {

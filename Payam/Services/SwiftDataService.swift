...
final class SwiftDataService: FeedDataService {
    ...
    /// Called once from `PayamApp.init()` after the ModelContainer is created.
    ///
    ...
    func configure(container: ModelContainer) {
        ...
        // Clustering is now owned by the River pipeline (SemanticClusterService).
        // The cache already contains clusterID/isCanonical from the previous
        // session's pipeline runs; running the legacy NLEmbedding clusterer
        // here would overwrite those with a different (inferior) grouping.
    ...
    /// Returns Sendable snapshots of every enabled FilterRuleModel.
    /// Safe to hand off to the background pipeline queue — the @Model itself stays on MainActor.
    @MainActor
    func loadFilterRuleSnapshots() -> [FilterRuleSnapshot] {
        guard let context = modelContext else { return [] }
        return FilterRuleService.loadEnabledRuleSnapshots(context: context)
    }

...
final class RiverSnapshotService: @unchecked Sendable {
    ...
    ///   - filterRules: Sendable snapshots of the user's active filter rules. Applied
    ///     to individual FeedItems before clustering so matched stories are absent
    ///     from both standalone view AND any cluster they would have joined.
    ///   - sourceFilterMeta: `sourceID → (feedURL, folderName)`. Required for any rule
    ///     whose scope is `folder:` or `feed:`. Items whose source is missing from this
    ///     map are evaluated under global rules only.
    ...
    func assembleSnapshot(
        ...
        let rawRiverItems = store.fetchRiverItemsAllHistory()

        // Apply AI-generated filter rules early so they affect clustering too.
        let riverItems = FilterRuleService.filter(
            items: rawRiverItems,
            sourceMap: sourceFilterMeta,
            rules: filterRules
        )
        let suppressedCount = rawRiverItems.count - riverItems.count
        ...
        DebugLog.log("H5", "RiverSnapshotService.swift:121", "snapshot.assembled", [
            ...
            "suppressedByFilterRules": suppressedCount,

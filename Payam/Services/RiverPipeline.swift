...
final class RiverPipeline: @unchecked Sendable {
    ...
    /// Refreshed by callers before each cycle via `runCycle(...filterRules:...)`.
    private var lastFilterRules: [FilterRuleSnapshot] = []

    /// `sourceID → (feedURL, folderName)` map used by filter-rule scope resolution.
    /// Captured from the caller's `[Source]` snapshot at the start of each cycle.
    private var lastSourceFilterMeta: [UUID: (feedURL: String, folderName: String?)] = [:]
    ...
    ///
    /// - Parameter filterRules: Sendable snapshots of the user's active filter rules.
    ...
    func runCycle(
        ...
        // Cache the filter inputs so a subsequent `runScoringCycle()` reuses them
        // without callers having to pass them through every time.
        ...
        let (snapshot, snapshotMs) = timer.time("Stage5-Snapshot") {
            snapshotService.assembleSnapshot(
                ...
                filterRules: lastFilterRules,
                sourceFilterMeta: lastSourceFilterMeta
            )
        }
        _ = snapshotMs

...
struct PayamApp: App {
    ...
    private static func emergencyOPMLExport(schema: Schema) {
        do {
            // Open the old store read-only with migration disabled so we can
            // read whatever is still there without triggering the same failure.
            let readOnlyConfig = ModelConfiguration(
                schema: schema,
                isStoredInMemoryOnly: false,
                cloudKitDatabase: .none
            )
            // If this also throws, the store is too corrupted to salvage.
            ...
            // Copy to Documents so it survives the temp directory cleanup.

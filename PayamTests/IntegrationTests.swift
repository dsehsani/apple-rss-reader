...
//
//  Run with Cmd+U in Xcode or: xcodebuild test -scheme Payam
//
...
private func loadFixture(_ filename: String) -> Data {
    ...
    guard let data = try? Data(contentsOf: fileURL) else {
        fatalError("Could not load fixture '\(filename)' at \(fileURL.path). Ensure the file exists in PayamTests/Fixtures/")
    }
    return data
}
